...
class FeaturesExtractor:
    ...
    def extract_training(self, path, n_features=8):
        """
        ...
        # Initiate arrays
        training_features = np.empty((0,n_features))
        training_features2d = []
        ...
        for i, file in enumerate(ply_files):
            ...
            # Compute features for the points of the chosen indices and place them in a [1, 4] matrix
            ...
            features2d = np.column_stack(features2d)

            # Concatenate features / labels of all clouds
            training_features = np.vstack((training_features, features))
            training_features2d.append(features2d)
            training_labels = np.hstack((training_labels, labels[training_inds]))
        
        training_features2d = np.concatenate(training_features2d, axis=1).T
        return np.concatenate((training_features, training_features2d), axis=1) , training_labels
...
if __name__ == '__main__':
    ...
    f_extractor = MultiscaleFeaturesExtractor(training_path, [10])
